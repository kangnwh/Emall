...
def create_order():
    ...
    if user_order_form.validate_on_submit():
        ...
        order.client_order_id = datetime.datetime.now().strftime(
            "%y%m%d%H%M%S%f")  # int("{userid}{timeflag}".format(userid=current_user.user_id,timeflag=timeflag))
        ...
        order.total_price, order.unit_price, order.imprinting_prices, order.setup_cost, order.freight_cost = prices.get_prices(
            order.prod_quantity)
        ...
        return redirect(url_for("userRoute.user_orders",
                                type='ongoing'))  # render_template("reload_parent.html")#reload_parent.html
...
def supp_update_quote():
    ...
    if supp_update_quote_form.validate_on_submit():
        ...
        supplier_perfer_total = (supp_update_quote_form.supplier_perfer_unit_price.data * this_quote.prod_quantity + \
                                 ...
                                 supplier_perfer_freight_cost) * (
                                1 + prod_profit_rate)  # supp_update_quote_form.supplier_perfer_total.data
