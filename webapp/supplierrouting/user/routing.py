...
def order_search():
    ...
    if key_words:
        ...
        query_base = BaseQuery(Order_system,s).filter(Order_system.supplier_id == current_user.supplier_id)
...
def quote_search():
    ...
    if key_words:
        ...
        s = Session()
        query_base = BaseQuery(Quote_system,s).filter(Quote_system.supplier_id==current_user.supplier_id)
